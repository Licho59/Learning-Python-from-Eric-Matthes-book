...
class RandomWalk():
    ...
    def fill_walk(self):
        '''Wygenerowanie wszystkich punktów dla błądzenia losowego; wykonywanie kroków, aż do chwili osiągnięcia oczekiwanej liczby punktów.'''
        while len(self.x_values) < self.num_points:
            x_step, y_step = self.get_step()
            #x_step, y_step = RandomWalk.get_step()
            next_x = self.x_values[-1] + x_step
            next_y = self.y_values[-1] + y_step
            # Odrzucenie ruchów, które prowadzą do nikąd.
